...
sys.path.append("..") # don't remove this line or it complains module not found

from InterFaceGAN.models.stylegan_generator import StyleGANGenerator
from model.encoder.align_all_parallel import align_face

LANDMARKS_MODEL_URL = '/home/sai_k/DualStyleGAN/checkpoint/shape_predictor_68_face_landmarks.dat'

parser = argparse.ArgumentParser(description="Find the latent space representation of an input image.")
parser.add_argument("--image_path", help="Filepath of the image to be encoded.", required=True)
parser.add_argument("--dlatent_path", help="Filepath to save the dlatent (WP) at.", required=True)
...
parser.add_argument("--save_frequency", default=500, help="How often to save the images to video. Smaller = Faster.", type=int)
...
#TODO this functions is not currently used --> use if results are not very good
def align_image(args):
    predictor = dlib.shape_predictor(LANDMARKS_MODEL_URL)
    aligned_image = align_face(filepath=args.image_path, predictor=predictor)
